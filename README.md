...
Раздел **E2E Response Time** показывает цифры по времени ответа агента за
выбранный период (по таймрейнджу дашборда): число запросов, среднее, p50, p95, p99.

Раздел **Trace Explorer** даёт сквозную (end-to-end) картину одного запуска:
- **Trace Finder** — поиск трейса по исходному сообщению пользователя
  (поле `Search message`); из него копируется `session_id` в селектор `Trace`.
- **Gantt** — таймлайн трейса: каждый бар = шаг агента (stage / вызов тулзы /
  результат), сгруппированы по стадии, цвет = статус (`ok`/`error`/`pending`/…),
  ширина = время до следующего шага. Фильтры: `Trace`, `Tool status`, `Agent`.
- **Step Details** — потактовый тайминг шагов выбранного трейса.

Gantt и waterfall рисуются плагинами `marcusolsson-gantt-panel` и
`auxmoney-waterfall-panel`, которые ставятся на выкатке через `GF_INSTALL_PLUGINS`
