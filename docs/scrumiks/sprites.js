/*
 * Скрамики — пиксель-спрайты (canonical asset data).
 *
 * 16×16 чанковый пиксель-арт в стиле «buddy» из Claude Code.
 * Единый дизайн-язык: общий blob-силуэт (SCRUMIK_BASE) + per-species палитра + оверлей фич.
 *
 * Рендер: для каждой клетки берём символ оверлея (если не '.'), иначе символ базы.
 * Базовые ключи: o=контур, g=тело, d=тень, h=блик, W=белок глаза, P=зрачок (=контур).
 * Оверлей объявляет свои ключи в `opal`.
 *
 * Это превью-источник правды на время подбора стиля. После апрува переносим в
 * apps/web-ui/src/lib/scrumiks/ как TS + React-рендерер. См. docs/SCRUMIC_DESIGN.md.
 */
...
const DOTS = "................";
const dots = (n) => Array.from({ length: n }, () => DOTS);
...
const SCRUMIKS = [
  ...
  // ------------------------------------------------------------------ EPIC
  {
    ...
    rarity: "epic",
    chance: "4.5%",
    affinity: "⚡⚡ Скорость · 🔥 Выносливость",
...
if (typeof window !== "undefined") {
  window.SCRUMIK_BASE = SCRUMIK_BASE;
  window.SCRUMIKS = SCRUMIKS;
  window.SCRUMIK_RARITY = RARITY;
}
if (typeof module !== "undefined") {
  module.exports = { SCRUMIK_BASE, SCRUMIKS, RARITY };
}
